...
class polylai_Admin
{
	...
	public function handle_restart()
	{
		...
		if (isset($_POST['polylai_restart'])) {
			if (wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['polylai_restart_nonce'])), 'polylai_restart_nonce')) {
				delete_transient(polylai_Cron::LOCK_TRANSIENT_KEY);
				delete_transient(polylai_Cron::ACTIVITY_TRANSIENT_KEY);

				$logs = [];
				$table_name = $wpdb->prefix . 'postmeta';
				$wpdb->delete($table_name, ['meta_key' => POLYLAI_TRANSLATING_KEY]);
				$wpdb->delete($table_name, ['meta_key' => POLYLAI_PROCESSING_KEY]);
				$wpdb->delete($table_name, ['meta_key' => POLYLAI_NEED_TR_KEY]);
			} else {
				wp_nonce_ays();
			}
		}
